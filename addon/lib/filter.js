const { extractProvider, parseSize, extractSize } = require("./titleHelper.js");
...
const Providers = {
  ...
  options: [
    ...
    { key: "thepiratebay", label: "ThePirateBay" },
    { key: "kickasstorrents", label: "KickassTorrents" },
    { key: "torrentgalaxy", label: "TorrentGalaxy" },
    { key: "magnetdl", label: "MagnetDL" },
    { key: "horriblesubs", label: "HorribleSubs", anime: true },
    { key: "nyaasi", label: "NyaaSi", anime: true },
    { key: "tokyotosho", label: "TokyoTosho", anime: true },
    { key: "anidex", label: "AniDex", anime: true },
    { key: "rutor", label: "Rutor", foreign: "🇷🇺" },
    { key: "rutracker", label: "Rutracker", foreign: "🇷🇺" },
    { key: "comando", label: "Comando", foreign: "🇵🇹" },
    { key: "bludv", label: "BluDV", foreign: "🇵🇹" },
    { key: "torrent9", label: "Torrent9", foreign: "🇫🇷" },
    { key: "ilcorsaronero", label: "ilCorSaRoNeRo", foreign: "🇮🇹" },
    { key: "mejortorrent", label: "MejorTorrent", foreign: "🇪🇸" },
    { key: "wolfmax4k", label: "Wolfmax4k", foreign: "🇪🇸" },
    { key: "cinecalidad", label: "Cinecalidad", foreign: "🇲🇽" },
    { key: "besttorrents", label: "BestTorrents", foreign: "🇵🇱" },
  ],
};
...
function applyFilters(streams, config) {
  return [filterByProvider, filterByQuality, filterBySize].reduce(
    (filteredStreams, filter) => filter(filteredStreams, config),
    streams
  );
}
...
function filterByQuality(streams, config) {
  ...
  const filterOptions = QualityFilter.options.filter((option) =>
    filters.includes(option.key)
  );
  return streams.filter((stream) => {
    const streamQuality = stream.name.split("\n")[1];
    const bingeGroup = stream.behaviorHints?.bingeGroup;
    return !filterOptions.some((option) =>
      option.test(streamQuality, bingeGroup)
    );
  });
}
...
function filterBySize(streams, config) {
  ...
  const sizeLimit = parseSize(
    config.type === Type.MOVIE ? sizeFilters.shift() : sizeFilters.pop()
  );
  return streams.filter((stream) => {
